...
async def get_user_sessions(user_id: str, admin: dict = Depends(get_current_user)):
    if admin.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Bạn không có quyền truy cập")
    
    sessions = await sessions_collection.find({"user_id": user_id}).sort("created_at", -1).to_list(None)
    for session in sessions:
        session["_id"] = str(session["_id"])
        session["messages"] = [str(message_id) for message_id in session["messages"]]
    return sessions
